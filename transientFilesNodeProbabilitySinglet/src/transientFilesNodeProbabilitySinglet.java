...
public class transientFilesNodeProbabilitySinglet {

    public transientFilesNodeProbabilitySinglet () throws FileNotFoundException {
        File workDirectory = new File("../TRANS");
        for (int i=0;i<workDirectory.listFiles().length;i++) {
            System.setErr(new PrintStream(new File("../j-none_NodeProbabilitySinglet_arg-"+workDirectory.listFiles()[i].getName().substring(23,28)+".txt")));
            try {
                ...
                int X=14,Y=16,N=X*Y;
                double histogramRange[]=new double[]{-2,2}, rozdzielczosc=1000, cellDr=(histogramRange[1]-histogramRange[0])/rozdzielczosc;
                ...
                double multimersTableDNode[][] = new double[N][2]; for (int j=0;j<N;j++) for (int k=0;k<2;k++) multimersTableDNode[j][k]=0;
                int lineLicznik=0; String line; 
                while ((line=reader.readLine())!=null) {
                    long startTime=System.currentTimeMillis();
                    lineLicznik++; 
                    String configurationFile[] = line.split("\t");
                    System.out.println("stage #1: "+String.valueOf(i+1)+"/"+String.valueOf(workDirectory.listFiles().length)+", configuration: "+String.valueOf(lineLicznik)+"/"+String.valueOf(100));
                    ...
                    for (int j=0;j<multimersTableD.length;j++) { 
                        for (int k=0;k<2;k++) {
                            multimersTableD[j][k]-=SMCoord[k];
                            multimersTableDNode[j][k]+=multimersTableD[j][k];
                        } 
                    }
                ...
                for (int j=0;j<N;j++) for (int k=0;k<2;k++) multimersTableDNode[j][k]/=(double)lineLicznik;
                reader = new BufferedReader(new FileReader(workDirectory.listFiles()[i])); 
                lineLicznik=0;
                while ((line=reader.readLine())!=null) {
                    String configurationFile[] = line.split("\t");
                    lineLicznik++; 
                    System.out.println("stage #2: "+String.valueOf(i+1)+"/"+String.valueOf(workDirectory.listFiles().length)+", configuration: "+String.valueOf(lineLicznik)+"/"+String.valueOf(100));
                    double boxSize[] = new double[]{Double.parseDouble(configurationFile[1]),Double.parseDouble(configurationFile[2]),Double.parseDouble(configurationFile[3])};
                    latticeCoef=(boxSize[0]/(double)X+boxSize[1]/(double)Y/Math.sqrt(3)*2.0)/2.0;
                    String multimers[] = configurationFile[4].split("m");
                    double multimersTableD[][] = new double[multimers.length-1][3];
                    ...
                    for (int j=0;j<multimersTableD.length;j++) { 
                        ...
                        for (int k=0;k<6;k++) {
                            int x=(int)Math.round((multimersTableD[j][0]-multimersTableDNode[j][0]+ROkreguOpisanego*Math.cos(multimersTableD[j][2]+k*2.0*b)-histogramRange[0])/cellDr);
                            if (x>=0 && x<rozdzielczosc) {
                                int y=(int)Math.round((multimersTableD[j][1]-multimersTableDNode[j][1]+ROkreguOpisanego*Math.sin(multimersTableD[j][2]+k*2.0*b)-histogramRange[0])/cellDr);
    ...
    public static void main(String[] args) {        
        try {
            new transientFilesNodeProbabilitySinglet();
